...
def main():
    ...
    for e_nr in range(nr_of_experiments):
        print(f"\n\n\nfor experiment nr = {e_nr + 1} with MC_LEN = {MC_SAM_LEN}")
        ...
        length_of_S = S.shape[0]  # Update length_of_S after burnin 
        S = S - S.mean(axis=0)  # Center the sources -> the observed signals should also be centered
        ...
        try:
            continuity_moduli = ContinuityModuli(true_source = S, mixed_signal=X, A = A, k_0=120, MC_SAM_LEN=MC_SAM_LEN)
    ...
    line1 = ax1.plot(range(1, nr_of_experiments + 1), delta_hist, 
    ...
    line2 = ax1.plot(range(1, nr_of_experiments + 1), eps0_hist, 
    ...
    ax1.set_xticks(range(1, nr_of_experiments + 1))
