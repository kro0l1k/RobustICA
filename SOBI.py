...
def sobi_algo(X, num_lags=None, eps=1.0e-6, random_order=True):
    # from: https://github.com/davidrigie/sobi/blob/master/sobi/sobi.py
    ...
    def sobi(X, num_lags=None, eps=1.0e-6, random_order = True):
        ...
        d, n = X.shape
        if d > n:
            print(" should have passed X.T, not X, fixing it with a transpose")
            X = X.T
            n, d = X.shape
